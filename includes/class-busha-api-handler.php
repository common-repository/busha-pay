...
/**
 * Sends API requests to Busha Pay.
 */
class Busha_API_Handler {
	...
	public static $api_version = '2019-06-30';
	...
	public static function send_request( $endpoint, $params = array(), $method = 'GET' ) {
		self::log( 'Busha Pay Request Args for ' . $endpoint . ': ' . print_r( $params, true ) );
		$args = array(
			'method'  => $method,
			'headers' => array(
				'X-BP-Api-Key' => self::$api_key,
				'X-BP-Version' => self::$api_version,
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( in_array( $code, array( 200, 201 ), true ) ) {
				...
				$errors = array(
					...
					429 => 'Busha Pay API rate limit exceeded.',
				);
